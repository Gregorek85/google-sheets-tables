 ...
 */
class gsTable {
  constructor(sheet, defaultKey) {
    var data = sheet.getDataRange().getValues();
    var headers = data.shift();
    var defaultKey = defaultKey || headers[0];
    if (headers.indexOf(defaultKey) === -1)
      defaultKey = headers[0];
    /**
    ...
    */
    ...
    /**
    * Matches sheet keyField column value with respective item property and delete row. Note: in case many rows with the same keyField only the first one will get deleted.
    * @param item         {object}                           Item object. Object properties must match with sheet columns.
    * @param keyField     {string}   [keyField = defaultKey] Optional name of sheet column and item property name to match.
    * @returns success    {bool}                             Indicates if deletion was successful.
    */
    this.DeleteItem = function (item, keyField) {
      //validate arguments
      keyField = keyField || defaultKey;
      if (headers.indexOf(keyField) === -1)
        return false;
      ...
      if (rIndex < 2)
        return false;
      //delete row
      var bool = sheet.deleteRow(rIndex);
      return bool;
    };
  }
}
...
/**
 * Creates table object based on 2d array. 2d array must have column descriptions in first row.
 * Each table item represents a table row, item properties represent table columns.
 * @param data        {object}                             Source data (2d array)
 * @param defaultKey   {string} [defaultKey = first column] Optional column name, indicates default key field.
 * @returns GSTable    {object}                             Object, containing table items.
 */
class gsTableFromData {
  constructor(data, defaultKey) {
    var headers = data.shift();
    var defaultKey = defaultKey || headers[0];
    if (headers.indexOf(defaultKey) === -1)
      defaultKey = headers[0];
